...
class Aauth
{
	...
	public function initialize($module = ''): void
	{
		if (empty($module)) {
			return;
		}
		$this->CI->config->load('aauth');
		$this->config_vars = $this->CI->config->item($module, 'aauth');
		if (empty($this->config_vars)) {
			throw new Exception('Aauth not initialized properly for HMVC');
		}
		$this->aauth_db = $this->CI->load->database($this->config_vars['db_profile'], true);
	}
	...
	public function login($identifier, $pass, $remember = false, $totp_code = null)
	{
		...
		if (true == $this->config_vars['login_with_name']) {
		...
		if (true == $this->config_vars['totp_active'] and false == $this->config_vars['totp_only_on_ip_change'] and true == $this->config_vars['totp_two_step_login_active']) {
			if (true == $this->config_vars['totp_two_step_login_active']) {
		...
		if (true == $this->config_vars['totp_active'] and true == $this->config_vars['totp_only_on_ip_change']) {
			$query = null;
			$query = $this->aauth_db->where($db_identifier, $identifier);
			$query = $this->aauth_db->get($this->config_vars['users']);
			$totp_secret = $query->row()->totp_secret;
			...
			if ($query->num_rows() > 0 and !$totp_code) {
				if ($ip_address != $current_ip_address) {
					if (false == $this->config_vars['totp_two_step_login_active']) {
						$this->error($this->CI->lang->line('aauth_error_totp_code_required'));
		...
		if (0 != $query->num_rows() && $this->verify_password($password, $row->pass)) {
			...
			if (true == $this->config_vars['remove_successful_attempts']) {
	...
	public function control($perm_par = false)
	{
		...
		if (false == $perm_par) {
			...
			if (!$this->is_loggedin()) {
				...
				if (false !== $this->config_vars['no_permission']) {
					redirect($this->config_vars['no_permission']);
				}
			}
		} elseif (!$this->is_allowed($perm_id)) {
	...
	public function reset_password($ver_code)
	{
		...
		if ($query->num_rows() > 0) {
			...
			if (true == $this->config_vars['totp_active'] and true == $this->config_vars['totp_reset_over_reset_password']) {
	 ...
	 *
	 * @param bool|int $user_id User id to update or FALSE for current user
	 *
	 ...
	 *
	 * @return bool|int False if create fails or returns user id if successful
	 */
	public function create_user($email, $pass, $username = false)
	{
		...
		if ($this->user_exist_by_username($username) && false != $username) {
		...
		if (false != $username && !ctype_alnum(str_replace($this->config_vars['additional_valid_chars'], '', $username))) {
	 ...
	 *
	 * @param int         $user_id  User id to update
	 * @param bool|string $email    User's email address, or FALSE if not to be updated
	 * @param bool|string $pass     User's password, or FALSE if not to be updated
	 * @param bool|string $name     User's name, or FALSE if not to be updated
	 ...
	 *
	 * @param bool|int $user_id User id to get or FALSE for current user
	 *
	 ...
	 */
	public function send_verification($user_id): void
	{
		$query = $this->aauth_db->where('id', $user_id);
		$query = $this->aauth_db->get($this->config_vars['users']);
	...
	public function delete_user($user_id)
	{
		...
		if (false === $this->aauth_db->trans_status()) {
	 ...
	 *
	 * @param bool|string $email Email address for user
	 *
	 * @return int User id
	 */
	 ...
	 *
	 * @return bool|int Group id or FALSE on fail
	 */
	public function create_group($group_name, $definition = '')
	{
		$query = $this->aauth_db->get_where($this->config_vars['groups'], ['name' => $group_name]);
		...
		if ($query->num_rows() < 1) {
			...
			$insert_id = $this->aauth_db->insert_id();
			$this->precache_groups();
	...
	public function update_group($group_par, $group_name = false, $definition = false)
	{
		...
		if (false != $group_name) {
			$query = $this->aauth_db->get_where($this->config_vars['groups'], ['name' => $group_name]);
			if ($query->num_rows() < 1 || $query->row()->id == $group_id) {
	 ...
	 * @param bool|int   $user_id   User id, if not given current user
	 *
	 * @return bool
	 */
	 ...
	 *
	 * @return bool|int Permission id or FALSE on fail
	 */
	public function create_perm($perm_name, $definition = '')
	{
		$query = $this->aauth_db->get_where($this->config_vars['perms'], ['name' => $perm_name]);
	 ...
	 * @param bool|int $user_id  User id to check, or if FALSE checks current user
	 *
	 * @return bool
	 */
	 ...
	 *
	 * @param int             $perm_par  Permission id or name to check
	 * @param bool|int|string $group_par Group id or name to check, or if FALSE checks all user groups
	 *
	 * @return bool
	 */
	public function is_group_allowed($perm_par, $group_par = false)
	{
		...
		if (false != $group_par) {
			...
			if (0 == strcasecmp($group_par, $this->config_vars['admin_group'])) {
				return true;
			}
	 ...
	 *
	 * @param bool|int|string $group_par Group id or name to allow
	 * @param int             $perm_par  Permission id or name to allow
	 *
	 * @return bool Allow success/failure
	 */
	 ...
	 *
	 * @param bool|int|string $group_par Group id or name to deny
	 * @param int             $perm_par  Permission id or name to deny
	 *
	 * @return bool Deny success/failure
	 */
	...
	public function delete_pm($pm_id, $user_id = null)
	{
		...
		if ($user_id == $result->sender_id) {
			if (1 == $result->pm_deleted_receiver) {
				return $this->aauth_db->delete($this->config_vars['pms'], ['id' => $pm_id]);
			}

			return $this->aauth_db->update($this->config_vars['pms'], ['pm_deleted_sender' => 1], ['id' => $pm_id]);
		}
		if ($user_id == $result->receiver_id) {
			if (1 == $result->pm_deleted_sender) {
				return $this->aauth_db->delete($this->config_vars['pms'], ['id' => $pm_id]);
			}

			return $this->aauth_db->update($this->config_vars['pms'], ['pm_deleted_receiver' => 1, 'date_read' => date('Y-m-d H:i:s')], ['id' => $pm_id]);
		}
	}
	 ...
	 *
	 * @param bool|int $receiver_id User id for message receiver, if FALSE returns for current user
	 *
	 ...
	 */
	public function set_as_read_pm($pm_id): void
	{
		$data = [
	 ...
	 */
	public function error($message = '', $flashdata = false): void
	{
		$this->errors[] = $message;
		if ($flashdata) {
	 ...
	 */
	public function keep_errors($include_non_flash = false): void
	{
	 ...
	 */
	public function print_errors($divider = '<br />'): void
	{
		$msg = '';
	 ...
	 * @param bool   $flashdata if TRUE add $message to CI flashdata (deflault: FALSE)
	 */
	public function info($message = '', $flashdata = false): void
	{
		$this->infos[] = $message;
		if ($flashdata) {
	 ...
	 * @see http://ellislab.com/codeigniter/user-guide/libraries/sessions.html
	 *
	 * @param bool $include_non_flash TRUE if it should stow basic infos as flashdata (default = FALSE)
	 */
	public function keep_infos($include_non_flash = false): void
	{
		// NOTE: keep_flashdata() overwrites anything new that has been added to flashdata so we are manually reviving flash data
	 ...
	 */
	public function print_infos($divider = '<br />'): void
	{
		$msg = '';
	 ...
	 */
	public function clear_infos(): void
	{
		$this->infos = [];
	...
	public function set_user_var($key, $value, $user_id = false)
	{
		...
		// if var not set, set
		if (false === $this->get_user_var($key, $user_id)) {
			$data = [
				'data_key' => $key,
				'value' => $value,
				'user_id' => $user_id
	 ...
	 * @param int $user_id ; if not given current user
	 *
	 * @return array|bool , FALSE if var is not set, the value of var if set
	 */
	 ...
	 */
	private function precache_perms(): void
	{
		$query = $this->aauth_db->get($this->config_vars['perms']);
	 ...
	 */
	private function precache_groups(): void
	{
		$query = $this->aauth_db->get($this->config_vars['groups']);
